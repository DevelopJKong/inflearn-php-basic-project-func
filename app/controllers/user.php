...
function create()
{
    return views('auth', ['requestUrl' => '/user/register']);
}

function store()
{
    return base(function ($args) {
        return createUser(...array_values($args)) ? redirect("/auth/login") : reject();
    });
}

function edit()
{
    return views('auth', ['requestUrl' => '/user/update', 'email' => user()['email']]);
}
